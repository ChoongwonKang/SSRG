...
def main():
    ...
    # 3-shot examples
    with open("3shot_dataset.jsonl", "r") as f:
        fewshot = [json.loads(line.strip()) for line in f if line.strip()]

    for idx, ex in enumerate(tqdm(ds, desc="Generating")):
        combined = []
        for example in fewshot:
            combined.extend(example["messages"])
        combined.extend(ex["messages"])  
        text = processor.apply_chat_template(combined, tokenize=False, add_generation_prompt=True)
        image_inputs, video_inputs, video_kwargs = process_vision_info(combined, return_video_kwargs=True)
        ...
        print(f"🔍 [{idx}] Video sources: {video_sources}")
        if video_inputs is not None:
            print(f"🎞️  [{idx}] Loaded {len(video_inputs)} video(s)")
        else:
            print(f"⚠️  [{idx}] No video loaded.")
        ...
        reference = next((m["content"] for m in ex["messages"] if m["role"] == "assistant"), "").strip()
